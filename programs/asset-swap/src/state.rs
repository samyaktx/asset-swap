...
use crate::error::AssetSwapProgramError;
...
impl<'info> LiquidityPoolAccount<'info> for Account<'info, LiquidityPool> {
    ...
    fn check_asset_key(&self, key: &Pubkey) -> Result<()> {
        if self.assets.contains(key) {
            ...
            Err(AssetSwapProgramError::InvalidAssetKey.into())
        }
    }
    ...
    fn fund(
        ...
        process_transfer_to_pool(from, to, amount, authority, mint, token_program)?;
        Ok(())
    }
    ...
    fn process_swap(
        ...
        // Process the swap
        if receive_amount == 0 {
            Err(AssetSwapProgramError::InvalidSwapNotEnoughPay.into())
        } else {
            process_transfer_to_pool(payer_pay, pool_pay, pay_amount, authority, mint, token_program)?;
...
fn determine_swap_receive(
    ...
    if r > big_r {
        return Err(AssetSwapProgramError::InvalidSwapNotEnoughLiquidity.into());
    }
